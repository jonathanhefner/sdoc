...
describe SDoc::Helpers do
  ...
  describe "#link_to_unless" do
    it "returns the opposite result of #link_to_if" do
      args = ["Bar < Foo", "qux", title: "Foo > Bar"]
      _(@helpers.link_to_unless(true, *args)).must_equal @helpers.link_to_if(false, *args)
      _(@helpers.link_to_unless(false, *args)).must_equal @helpers.link_to_if(true, *args)
    end
  end
